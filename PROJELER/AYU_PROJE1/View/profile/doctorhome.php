                    ...
                    <div class="row"> <!-- Content Row -->
                        ...
                        <form action="" method="POST" id="profileEditForm" class="">
                                    <div class="form-group row">
                                        ...
                                        <input type="text" name="tcnumber" class="form-control form-control-user col-md-9" id="tcnumber" value="<?= $data['doctor']['doctorTCNumber'] ?? NULL ;?>" disabled>
                                    </div>
                                    <div class="form-group row">
                                        <label class="col-md-3" for="name">Adı</label>
                                        <input type="text" name="name" class="form-control form-control-user col-md-9" id="name" aria-describedby="name" value="<?= Helper::convertLetter($data['doctor']['doctorName'],'firstUpper') ?? NULL ;?>">
                                    </div>
                                    <div class="form-group row">
                                        <label class="col-md-3" for="surname">Soyadı</label>
                                        <input type="text" name="surname" class="form-control form-control-user col-md-9" id="surname" value="<?= Helper::convertLetter($data['doctor']['doctorSurname'],'upper') ?? NULL ;?>">
                                    </div>                                  
                                    <div class="form-group row">
                                        <span class="col-md-3">Cinsiyet </span>
                                        <div class="form-group col-md-9">
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="doctorGender" id="female" value="K" <?= ($data['doctor']['doctorGender'] == 'K') ? 'checked' : NULL ;?> disabled>
                                                <label class="form-check-label" for="female">Kadın</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="doctorGender" id="male" value="E" <?= ($data['doctor']['doctorGender'] == 'E') ? 'checked' : NULL ;?> disabled>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <select class="form-select form-control col-md-4" name="cityName" id="cityName" aria-label="Default select example" disabled>
                                            ...
                                            ?>
                                            <option value="<?= $value['cityID'] ; ?>" <?= ($data['doctor']['doctorBirthCity'] == $value['cityID']) ? 'selected' : NULL ; ?> ><?= $value['cityName'] ; ?></option>
                                            <?php 
                                                } 
                                            ?>
                                        </select>                                    
                                        <select class="form-select form-control col-md-4" name="townName" id="townName" aria-label="Default select example" disabled>
                                            <option value="<?= $data['doctor']['doctorBirthTown'] ?? NULL ;?>" selected ><?= $data['doctor']['doctorBirthTown'] ?? NULL ;?></option>   
                                            <?php 
                                                foreach($data['towns'] as $key => $value){
                                            ?>
                                            <option value="<?= $value['townID'] ; ?>" <?= ($data['doctor']['doctorBirthTown'] == $value['townID']) ? 'selected' : NULL ; ?>><?= $value['townName'] ; ?></option>
                                            <?php 
                                                } 
                                            ?>                                    
                                        </select>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <input type="date" name="birthday" class="form-control form-control-user col-md-3" disabled id="birthday" value="<?= $data['doctor']['doctorBirthday'] ?? NULL ;?>">
                                    </div> 
                                    <div class="form-group row">
                                        ...
                                        <select class="form-select form-control col-md-4" name="cityAddress" id="cityAddress" aria-label="Default select example">
                                            ...
                                            <?php 
                                                foreach($data['cities'] as $key => $value){
                                            ?>
                                            <option value="<?= $value['cityID'] ; ?>" <?= ($data['doctor']['doctorAddressCity'] == $value['cityID']) ? 'selected' : NULL ; ?>><?= $value['cityName'] ; ?></option>
                                            <?php 
                                                } 
                                            ?>
                                        </select>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <textarea class="form-control col-md-9" id="address" name="address"><?= Helper::convertLetter($data['doctor']['doctorAddress'],'upperWords') ?? NULL ;?></textarea>
                                    </div>
...
$(function(){ //jQuery START
    ...
    profileEditForm.addEventListener('submit', (e) => {
        ...
        $.ajax({
            type:'post',
            url: SITE_URL + '/Controller/api.php?process=doctorEditProfile',
            data:myData,
            dataType :'json',
            success:function(resultData){          
                $(".myload").html('') ;
    ...
    passwordEditForm.addEventListener('submit', (e) => {
        ...
        $.ajax({
            type:'post',
            url: SITE_URL + '/Controller/api.php?process=doctorEditPassword',
            data:myData,
            dataType :'json',
            success:function(resultData){          
                $(".myloadPass").html('') ;
