...
<!-- class.upload.php 2716 satırındaki getsize() fonksiyonun içeriği başltılınca bazı yerler hata vermeden çalıştı bazı yerler hata vermeye devam etti -->
...
<button class="btn btn-primary" name="upp" type="submit"><i class="fa fa-fw fa-lg fa-check-circle"></i>Kayıt Güncelle</button>&nbsp;&nbsp;&nbsp;<a class="btn btn-secondary" href="<?php echo admin;?>"><i class="fa fa-fw fa-lg fa-times-circle"></i>Ana Sayfaya Dön</a>
    ...
    case 'contact':
        ...
        if(isset($_POST['upp'])){
            ...
            $map      = $_POST['map'];#### etiketler silinmesin diye AMA buşekişlde de olsa bir güvenlik açığıdır
...
<!-- burda gönderilecek mail içeriğini kendimiz belirliyoruz -->
<form action="" method="POST">
                ...
                if($order->rowCount()){
                    ...
                    if($result){
                        ...
                        if($email == 1){
                            ...
                            $mailcontent      = "
                            <p><b>Siparişiniz hakkında değişiklik meydana geldi siparişinizin yeni durumu:<br></b>".$dqueryrow->durumbaslik."</p>"; #### içerik kısmı panelden textarea alanına yazılıp burdan alınabilir
...
<script>
    ...
    case 'productimagedelete':
        ...
        if($query->rowCount()){
            ...
            $result = $del->execute([':k' => $id]);
            if($result){ 
                @unlink("../uploads/product/".$row->resimdosya);#### önce veritabanından silinir sonra klasörden silinir ki tabloda gözükme
                go($_SERVER['HTTP_REFERER']);
            }else{
                alert("Hata oluştu","danger");
            }
                ...
                if($already->rowCount()){
                    alert("Bu kategori zaten kayıtlı","danger");
                        ...
                        if($image->processed){
                            ...
                            @unlink("../uploads/".$row->katresim);#### eski foto silinir
...
if(isset($_POST['upp'])){
    ...
    if($image->uploaded){
        ...
        if($image->processed){
            ...
            if($up){
                @unlink("../uploads/customer/".$row->bayilogo); #### güncelleme başarılı ise eski foto silinir. yer tutmasın diye
...
/** silinmez kategori mantığı = diğer categorilerden biri silindiği anda o kategoriye ait tüm rürünlerin kategorisi silinmeyen kategori olacaktır */
case 'categorydelete':
$id = get('id');
if(!$id){
go(admin);
...
#### genelde silinme değilde pasife alınma yapılması ileride yaşanacak bir problemde sorun problem çıkmması için  
case 'bankdelete':
$code = get('id');
if(!$code){
go(admin);
...
$pcontent   = $_POST['pcontent']; /* ckeditor içeriğine olduğu gibi alınsın diye security'den geçirişlmedi   */ 
...
$name   = post('name');
$seourl = post('seourl');### kişi seo linki girerse şayet ama şart değil
if(!$seourl){
$sef = sef_link($name);
}else{
$sef = $seourl;
